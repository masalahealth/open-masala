...
def get_reference(analyte: str) -> dict:
    ...
    citations, and overclaim guard. Matches a LOINC code, id, or name."""
...
def _row_for_eval(analyte: str, sex: Optional[str]):
    ...
    if sex:
        s = sex.strip().lower()
        sexed = [r for r in hits if r["sex"] == "any"
...
def evaluate_value(analyte: str, value: float, sex: Optional[str] = None,
    ...
    rationale and overclaim guard. STATELESS: the value is used only to compute
    this response; nothing is stored. A reference lookup, not a diagnosis."""
    ...
    if target and target[0] == "cmp":
        ...
        if flagged is not None:
            ...
            explanation = (f"{value} {comparator} {thresh} → "
                           f"{'meets' if flagged else 'does not meet'} the concern threshold "
                           f"for South Asian ({row['direction_of_risk']}-is-risk).")
        ...
        explanation = f"This row is not a simple numeric threshold ({row['measure_type']}); see guidance."

    return {
        "analyte": row["analyte"], "loinc": row.get("loinc"), "value": value,
        "unit": row["unit"], "sex_used": sex, "status": status, "explanation": explanation,
        ...
        "notes": row.get("notes"), "usage_rules": USAGE_RULES, "disclaimer": DISCLAIMER,
    }
